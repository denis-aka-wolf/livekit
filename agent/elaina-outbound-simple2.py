...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession(
        vad=silero.VAD.load(),
        # Пока закомментируйте STT и TTS
        # stt=openai.STT(...),
        # tts=cartesia.TTS(),
        llm=openai.LLM(
    ...
    try:
        logger.info(f"Dialing {phone_number} via SIP...")
        ...
        # Просто ждем появления участника и ничего не делаем
        participant = await ctx.wait_for_participant(identity=participant_identity)
        logger.info(f"SUCCESS: Participant {participant.identity} joined the room.")
        
        # Держим звонок активным 30 секунд для теста связи
        await asyncio.sleep(30)
        logger.info("Test finished, hanging up...")
