...
class OutboundCaller(Agent):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            instructions=f"""
                "Ты голосовой ассистент, говоришь по-русски. "
                "Когда абонент отвечает на звонок, вежливо поприветствуй его и коротко представься. "
                "Не затягивай диалог, можно просто сказать: "
                "«Здравствуйте, это тестовый звонок, проверяем связь»." """
        )
...
async def entrypoint(ctx: JobContext):
    ...
    # the following uses local Whisper STT, Kokoro TTS и Qwen (llama.cpp)
    session = AgentSession(
        ...
        tts=openai.TTS(
            base_url="http://127.0.0.1:8880/v1",  # твой kokoro-fastapi
            model="kokoro",
            voice="af_heart",                     # тот же, что ты тестировал в curl
            api_key="no-key-needed",
        ),
        llm=openai.LLM(
